...
// slightly different splatting to handle big splatSizes
void clayxelVertFoliage(uint vId, float splatSizeMult, float normalOrientedSplat, out float4 tex, out float3 vertexColor, out float3 outVertPos, out float3 outNormal){
	...
	#if !defined(SHADERPASS_SHADOWCASTER) 
		float3 eyeVec;
		if(splatSizeMult > 1.0){
			eyeVec = UNITY_MATRIX_V._m20_m21_m22;
		}
		else{
			eyeVec = normalize(_WorldSpaceCameraPos - p);
		}
...
// In VR we can't use the camera as a stable reference space because of head-tilting. 
// We use object coordinates instead.
void clayxelVertVR(uint vId, float splatSizeMult, float normalOrientedSplat, out float4 tex, out float3 vertexColor, out float3 outVertPos, out float3 outNormal){
#ifdef CLAYXELS_VALID
	int4 clayxelPointData = chunkPoints[vId / 3];
	...
	float3 axis1 = normalize(UNITY_MATRIX_M._m00_m01_m02);
	float3 axis2 = normalize(UNITY_MATRIX_M._m10_m11_m12);
	float3 axis3 = normalize(UNITY_MATRIX_M._m20_m21_m22);

	// check which axis is the least orthogonal to the normal and discard it
	float d1 = abs(dot(axis1, outNormal));
	float d2 = abs(dot(axis2, outNormal));
	float d3 = abs(dot(axis3, outNormal));

	if(d1 > d2 && d1 > d3){// discard axis1
		refUpVec = axis3;
		refSideVec = axis2;
	}
	else if(d2 > d1 && d2 > d3){// discard axis2
		refUpVec = axis1;
		refSideVec = axis3;
	}
	else{// discard axis3
	...
	float3 sideVec = normalize(cross(refUpVec, outNormal)) * (newSplatSize*2.0);
	float3 upVec = normalize(cross(refSideVec, outNormal)) * newSplatSize;

	if(dot(cross(sideVec, upVec), outNormal) > 0.0){
		sideVec = -sideVec;
	}
	...
	#if !defined(SHADERPASS_SHADOWCASTER) 
		if(splatSizeMult > 1.0){
			float3 eyeVec = UNITY_MATRIX_V._m20_m21_m22;
			outVertPos = outVertPos - (eyeVec * (dot(eyeVec, outVertPos - p)));
		}
		else{
			float3 eyeVec = normalize(_WorldSpaceCameraPos - p);
